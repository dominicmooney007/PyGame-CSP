...
def main():
    ...
    while running:
        ...
        mouse_in_circle = is_point_in_circle(mouse_x, mouse_y, center_x, center_y, circle_size)
        
        # Display function information and return values
        font = pygame.font.Font(None, 24)
        info_texts = [
            f"Mouse position: ({mouse_x}, {mouse_y})",
            f"Distance to center: {distance:.1f}",
            f"Circle size: {circle_size}",
            f"Mouse in circle: {mouse_in_circle}",
            f"Circle color based on distance"
        ]
        
        for i, text in enumerate(info_texts):
            color = BLACK
            if i == 3 and mouse_in_circle:  # Highlight when mouse is in circle
                color = RED
            rendered_text = font.render(text, True, color)
            screen.blit(rendered_text, (10, 50 + i * 25))
        
        # Draw a line from center to mouse to visualize distance
        pygame.draw.line(screen, GRAY, (center_x, center_y), (mouse_x, mouse_y), 2)
        
        # Show function code examples
        code_font = pygame.font.Font(None, 18)
        code_bg = pygame.Surface((280, 200))
        code_bg.fill((240, 240, 240))
        screen.blit(code_bg, (500, 50))
        
        code_lines = [
            "Function Examples:",
            "",
            "def draw_house(x, y, width, height, colors):",
            "    # Use parameters to draw house",
            "    pygame.draw.rect(screen, color, ...)",
            "",
            "def calculate_distance(x1, y1, x2, y2):",
            "    distance = sqrt((x2-x1)² + (y2-y1)²)",
            "    return distance  # Returns a value!",
            "",
            "# Calling functions:",
            "draw_house(50, 400, 120, 100, RED, YELLOW)",
            "dist = calculate_distance(0, 0, 10, 10)"
        ]
        ...
        for i, line in enumerate(code_lines):
            ...
            screen.blit(text, (505, 55 + i * 15))
        
        # Show parameter vs return value explanation
        explanation_font = pygame.font.Font(None, 20)
        explanations = [
            "Parameters go IN to functions (inputs)",
            "Return values come OUT of functions (outputs)",
            "Functions without return give back None",
            "Functions with return give back calculated values"
        ]
    ...
    pygame.quit()
    print("Function examples completed!")
    print("You learned about:")
    print("- Defining functions with parameters")
    print("- Functions that return values vs functions that don't")
    print("- Using parameters to customize function behavior")
    print("- Code reuse - same function, different parameters")
    print("- Boolean return values for conditions")
